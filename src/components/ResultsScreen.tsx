  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-4 py-8 sm:py-12">
            ...
            <section key={id} className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
              <div className="flex items-baseline justify-between">
                <h2 className="text-lg font-bold text-slate-50">
                  {participant.name}
                  {formationId && <span className="ml-2 text-xs font-normal text-slate-500">{formationId}</span>}
                </h2>
      ...
      <div className="mt-10 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
        <button
          onClick={goToBracket}
          className="w-full sm:w-auto rounded-xl bg-emerald-500 px-8 py-3.5 text-base font-bold text-emerald-950 shadow-lg shadow-emerald-500/20 hover:bg-emerald-400 active:scale-[0.98] transition"
        >
          Ir para o chaveamento 🎲
        </button>
        <button
          onClick={resetAll}
          className="w-full sm:w-auto rounded-xl border border-slate-700 bg-slate-900 px-8 py-3.5 text-base font-semibold text-slate-200 hover:bg-slate-800 active:scale-[0.98] transition"
        >
          Começar do zero
        </button>
      </div>
